...
function CreateHomeworkContent() {
  ...
  async function loadData() {
    ...
    if (assignmentId) {
      ...
      if (assignment) {
        ...
        setDueDate(assignment.due_at ? new Date(assignment.due_at).toISOString().slice(0, 10) : "");
      }
      ...
      const formatted = (questionRows || []).map((q) => ({
        id: q.id,
  ...
  async function saveAssignment(publish: boolean) {
    ...
    let assignmentIdToUse = assignmentId;
    ...
    if (assignmentIdToUse) {
      ...
      await supabase.from("homework_questions").delete().eq("assignment_id", assignmentIdToUse);
    } else {
      ...
      assignmentIdToUse = assignment.id;
    }
    ...
    if (publish && assignmentIdToUse) {
      ...
      const existingIds = new Set((existingSubmissions || []).map((s) => s.student_id));
      const newRows = (cohortStudents || [])
        .filter((student) => !existingIds.has(student.student_id))
        .map((student) => ({
          assignment_id: assignmentIdToUse,
      ...
      if (newRows.length > 0) {
        await supabase.from("homework_submissions").insert(newRows);
      }
    }
