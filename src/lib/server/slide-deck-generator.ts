...
function normalizeShortList(
  ...
  const source = Array.isArray(value)
    ? value
        ...
        .filter(Boolean)
        .map((item) => trimToWordLimit(item, maxWordsPerItem))
    : [];

  const normalized = source.slice(0, maxItems);
  let fallbackIndex = 0;
  while (normalized.length < minItems && fallbackIndex < fallbackItems.length) {
    normalized.push(trimToWordLimit(fallbackItems[fallbackIndex], maxWordsPerItem));
    fallbackIndex += 1;
  }

  while (normalized.length < minItems) {
    normalized.push(trimToWordLimit(`Point ${normalized.length + 1}`, maxWordsPerItem));
  }
...
function ensureSingleQuestion(text: string, fallbackQuestion: string): string {
  const candidate = safeText(text, fallbackQuestion);
  const firstChunk = candidate.split(/[.?!؟]/)[0]?.trim();
  const base = firstChunk || fallbackQuestion.replace(/[?؟]+$/, "");
  return `${base}?`;
}

function extractEnglishWord(...values: Array<string | null | undefined>): string {
...
function extractArabicWord(...values: Array<string | null | undefined>): string {
  for (const value of values) {
    const match = value?.match(/[\u0600-\u06FF]+/);
    if (match?.[0]) {
      return match[0];
    }
  }
...
${JSON.stringify(
  ...
  function coerceDeckToPolicy(inputSlides: PolicySlide[]): PolicySlide[] {
    const explanationSlideCount = Math.max(slideCount - 6, 0);
    ...
    const expectedTypes: PolicySlide["type"][] = [
      "title",
      "key_points",
      ...Array.from({ length: explanationSlideCount }, () => "content" as const),
      "activity",
      "quiz_preview",
      "question_answer",
      "summary",
    ...
    const expectedPhases: NonNullable<PolicySlide["lesson_phase"]>[] = [
      "title",
      "objectives",
      ...Array.from({ length: explanationSlideCount + 1 }, () => "core_teaching" as const),
      "practice",
      "practice",
      "summary_goodbye",
    ];

    function defaultTitle(index: number, language: "ar" | "en"): string {
      const slot = expectedTypes[index];
      if (slot === "title") return language === "en" ? "Lesson Title" : "عنوان الدرس";
      if (slot === "key_points") return language === "en" ? "Learning Objectives" : "أهداف التعلم";
      if (slot === "activity") return language === "en" ? "Class Activity" : "نشاط صفي";
      if (slot === "quiz_preview") return language === "en" ? "Training Check 1" : "تدريب ١";
      if (slot === "question_answer") return language === "en" ? "Training Check 2" : "تدريب ٢";
      if (slot === "summary") return language === "en" ? "Summary" : "الخلاصة";
      return language === "en" ? `Explanation ${index - 1}` : `شرح ${index - 1}`;
    }

    function defaultBody(index: number, language: "ar" | "en"): string {
      const slot = expectedTypes[index];
      if (slot === "title") {
        return language === "en"
          ? "Welcome to today’s lesson."
          : "مرحباً بكم في درس اليوم.";
      }
      if (slot === "key_points") {
        return language === "en"
          ? "Today we will learn the main idea clearly and step by step."
          : "اليوم سنتعلم الفكرة الأساسية بوضوح خطوة بخطوة.";
      }
      if (slot === "activity") {
        return language === "en"
          ? "Let’s practice together using one simple activity."
          : "هيا نتدرب معاً من خلال نشاط بسيط.";
      }
      if (slot === "quiz_preview") {
        return language === "en"
          ? "Choose the correct answer?"
          : "اختر الإجابة الصحيحة؟";
      }
      if (slot === "question_answer") {
        return language === "en"
      ...
      if (slot === "summary") {
        return language === "en"
          ? "Great work today. Let’s review the key idea."
          : "عمل رائع اليوم. لنراجع الفكرة الأساسية.";
      }
      return language === "en"
        ? "We explain one key idea with a simple example."
        : "نشرح فكرة واحدة مهمة مع مثال بسيط.";
    }

    return Array.from({ length: slideCount }, (_, index) => {
      const source = inputSlides[index] ?? inputSlides[inputSlides.length - 1];
      const type = expectedTypes[index] ?? "content";
      const lessonPhase = expectedPhases[index] ?? "core_teaching";

      const titleEn = trimToWordLimit(
        safeText(source?.title_en, safeText(source?.title_ar, defaultTitle(index, "en"))),
        12
      );
      const titleAr = trimToWordLimit(
        safeText(source?.title_ar, safeText(source?.title_en, defaultTitle(index, "ar"))),
        12
      );

      let bodyEn = trimToWordLimit(safeText(source?.body_en, defaultBody(index, "en")), 28);
      let bodyAr = trimToWordLimit(safeText(source?.body_ar, defaultBody(index, "ar")), 28);

      if (lessonPhase === "practice") {
        bodyEn = trimToWordLimit(ensureSingleQuestion(bodyEn, defaultBody(index, "en")), 18);
        bodyAr = trimToWordLimit(ensureSingleQuestion(bodyAr, defaultBody(index, "ar")), 18);
      }

      const isPractice = lessonPhase === "practice";
      const isSummaryLike = type === "key_points" || type === "summary";
      const isQuestionAnswer = type === "question_answer";

      const slide: PolicySlide = {
        ...(source || {}),
        id: crypto.randomUUID(),
        sequence: index,
        is_required: true,
        ...
        body_en: bodyEn,
        body_ar: bodyAr,
        visual_hint: safeText(
          source?.visual_hint,
          type === "activity" || lessonPhase === "practice"
            ? "Simple child-friendly illustration supporting the prompt."
            : "Clean visual supporting the main teaching point."
        ),
        bullets_en: isSummaryLike
          ? normalizeShortList(source?.bullets_en, {
              minItems: 3,
              maxItems: 5,
              maxWordsPerItem: 12,
              fallbackItems: ["Key point one", "Key point two", "Key point three"],
            })
          : null,
        bullets_ar: isSummaryLike
          ? normalizeShortList(source?.bullets_ar, {
              minItems: 3,
              maxItems: 5,
              maxWordsPerItem: 12,
              fallbackItems: ["النقطة الأولى", "النقطة الثانية", "النقطة الثالثة"],
            })
          : null,
        reveal_items_en: isQuestionAnswer
          ? normalizeShortList(source?.reveal_items_en, {
              minItems: 1,
              maxItems: 3,
              maxWordsPerItem: 8,
              fallbackItems: ["True"],
          ...
          : null,
        reveal_items_ar: isQuestionAnswer
          ? normalizeShortList(source?.reveal_items_ar, {
              minItems: 1,
              maxItems: 3,
              maxWordsPerItem: 8,
              fallbackItems: ["صحيح"],
        ...
        layout:
          type === "diagram_description"
            ? "image_left"
            : type === "title" || type === "activity" || isPractice || type === "summary"
              ? "default"
              : "default",
        idea_focus_en: safeText(source?.idea_focus_en, titleEn),
        idea_focus_ar: safeText(source?.idea_focus_ar, titleAr),
        title_size: "md",
        body_size: "md",
        practice_question_count: isPractice ? 1 : null,
        ...
        speaker_notes_en: safeText(source?.speaker_notes_en, buildFallbackSpeakerNotes(source || {}, "en")),
        speaker_notes_ar: safeText(source?.speaker_notes_ar, buildFallbackSpeakerNotes(source || {}, "ar")),
      };

      if (index === firstTrainingSlideIndex) {
        slide.interaction_type = "choose_correct";
        slide.interaction_prompt_en = safeText(source?.interaction_prompt_en, "Choose the correct answer.");
        slide.interaction_prompt_ar = safeText(source?.interaction_prompt_ar, "اختر الإجابة الصحيحة.");
        slide.interaction_options_en = normalizeShortList(source?.interaction_options_en, {
          minItems: 3,
          maxItems: 4,
          maxWordsPerItem: 6,
          fallbackItems: ["Option 1", "Option 2", "Option 3"],
        });
        slide.interaction_options_ar = normalizeShortList(source?.interaction_options_ar, {
          minItems: 3,
          maxItems: 4,
          maxWordsPerItem: 6,
          fallbackItems: ["الخيار ١", "الخيار ٢", "الخيار ٣"],
        });
        slide.interaction_correct_index = 0;
      }

      if (index === secondTrainingSlideIndex) {
        slide.interaction_type = "true_false";
        slide.interaction_prompt_en = safeText(source?.interaction_prompt_en, "True or false?");
        slide.interaction_prompt_ar = safeText(source?.interaction_prompt_ar, "صح أم خطأ؟");
        slide.interaction_true_false_answer =
          typeof source?.interaction_true_false_answer === "boolean"
            ? source.interaction_true_false_answer
            : true;
      }

      if (subjectKey === "english" && lessonPhase === "core_teaching") {
        const vocabEn = safeText(
          source?.vocabulary_word_en,
          extractEnglishWord(slide.title_en, slide.body_en, slide.idea_focus_en)
        );
        const vocabAr = safeText(
          source?.vocabulary_word_ar,
          extractArabicWord(slide.title_ar, slide.body_ar, slide.idea_focus_ar)
        );
        slide.vocabulary_word_en = vocabEn;
        slide.vocabulary_word_ar = vocabAr;
        slide.say_it_twice_prompt = true;

        let englishBody = trimToWordLimit(slide.body_en, 20);
        if (!containsNormalized(englishBody, "say it twice")) {
          englishBody = `${englishBody} Say it twice: ${vocabEn}.`;
        }
        if (!containsNormalized(englishBody, vocabEn)) {
          englishBody = `${englishBody} ${vocabEn}.`;
        }
        slide.body_en = trimToWordLimit(englishBody, 28);

        let arabicBody = trimToWordLimit(slide.body_ar, 22);
        if (!containsNormalized(arabicBody, vocabAr)) {
          arabicBody = `${arabicBody} الكلمة: ${vocabAr}.`;
        }
        slide.body_ar = trimToWordLimit(arabicBody, 28);
      }

      if (subjectKey === "math") {
        if (lessonPhase === "core_teaching") {
          slide.representation_stage = index === 2 ? "concrete_visual" : "abstract";
        } else {
          slide.representation_stage = "not_applicable";
        }
      }

      if (!isPractice) {
        slide.practice_question_count = null;
      }
...
${buildSpeakerNotesContext(slides)}`;
  ...
  while (validationIssues.length > 0 && repairAttempts < maxRepairAttempts) {
  ...
  if (validationIssues.length > 0) {
    const coercedSlides = coerceDeckToPolicy(slides);
    const coercedIssues = validateGeneratedSlides(coercedSlides, {
      slideCount,
      subjectKey,
    });

    if (coercedIssues.length === 0) {
      slides = coercedSlides;
    } else {
      const issueSummary = coercedIssues.slice(0, 3).join(" ");
      throw new SlideGenerationError(
        issueSummary
          ? `Generated slides did not pass the mandatory lesson policy. ${issueSummary}`
